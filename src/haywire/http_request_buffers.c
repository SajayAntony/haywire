...
KHASH_MAP_INIT_INT64(pointer_hashmap, unsigned long)
...
void http_request_buffer_pin(hw_request_buffer* buf, unsigned long id, void* pointer) {
    http_request_buffer* buffer = (http_request_buffer*) buf;

    khiter_t offset_key = kh_get(pointer_hashmap, buffer->offsets, id);
    ...
    if (is_missing) {
        offset_key = kh_put(pointer_hashmap, buffer->offsets, id, &ret);
    } 

    kh_value(buffer->offsets, offset_key) = offset;
}

void http_request_buffer_reassign_pin(hw_request_buffer* buf, unsigned long old_id, unsigned long new_id) {
    http_request_buffer* buffer = (http_request_buffer*) buf;

    khiter_t old_offset_key = kh_get(pointer_hashmap, buffer->offsets, old_id);
    ...
    if (!is_missing) {
        ...
        khiter_t new_offset_key = kh_put(pointer_hashmap, buffer->offsets, new_id, &ret);
        kh_value(buffer->offsets, new_offset_key) = offset;
        old_offset_key = kh_get(pointer_hashmap, buffer->offsets, old_id);
        kh_del(pointer_hashmap, buffer->offsets, old_offset_key);
    }
}

void* http_request_buffer_locate(hw_request_buffer* buf, unsigned long id, void* default_pointer) {
    http_request_buffer* buffer = (http_request_buffer*) buf;
