 ...
 * the caller eventually, when http_request_buffer_locate is called. The pin is given an id by the caller so it can be
 * retrieved later. Pins will be overwritten if the same id is used multiple times.
 */
void http_request_buffer_pin(hw_request_buffer* buffer, unsigned long id, void* pointer);

/**
 * Allows the caller to assign a new id to a pin.
 */
void http_request_buffer_reassign_pin(hw_request_buffer* buffer, unsigned long old_id, unsigned long new_id);

/**
 * Returns the pointer to the memory region associated with a pin. If there isn't a pin with that id, then
 ...
 */
void* http_request_buffer_locate(hw_request_buffer* buffer, unsigned long id, void* default_pointer);
