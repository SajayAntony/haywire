...
int http_request_on_url(http_parser *parser, const char *at, size_t length)
{
    ...
    if (url->length) {
        /* Another URL chunk, not necessarily contiguous to the other URL chunks seen before, so we only increment
         * the length. When the URL is located later on, it will be guaranteedly contiguous. */
        url->length += length;
    } else {
        url->value = at;
        url->length = length;
...
int http_request_on_header_field(http_parser *parser, const char *at, size_t length)
{
    ...
    if (connection->current_header_key.length > 0) {
        /* Another header key chunk, not necessarily contiguous to the other chunks seen before, so we only increment
         * the length. When the header key is located later on, it will be guaranteedly contiguous. */
